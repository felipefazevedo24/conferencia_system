...
def upgrade():
    ...
    if not _has_table(inspector, 'localizacao_armazem'):
        op.create_table(
            'localizacao_armazem',
    ...
    inspector = sa.inspect(bind)
    if _has_table(inspector, 'localizacao_armazem') and not _has_index(inspector, 'localizacao_armazem', 'ix_localizacao_armazem_codigo'):
        op.create_index('ix_localizacao_armazem_codigo', 'localizacao_armazem', ['codigo'])

    if not _has_table(inspector, 'item_wms'):
        op.create_table(
            'item_wms',
            sa.Column('id', sa.Integer(), nullable=False),
    ...
    inspector = sa.inspect(bind)
    if _has_table(inspector, 'item_wms') and not _has_index(inspector, 'item_wms', 'ix_item_wms_numero_nota'):
        op.create_index('ix_item_wms_numero_nota', 'item_wms', ['numero_nota'])
    inspector = sa.inspect(bind)
    if _has_table(inspector, 'item_wms') and not _has_index(inspector, 'item_wms', 'ix_item_wms_codigo_item'):
        op.create_index('ix_item_wms_codigo_item', 'item_wms', ['codigo_item'])
    inspector = sa.inspect(bind)
    if _has_table(inspector, 'item_wms') and not _has_index(inspector, 'item_wms', 'ix_item_wms_localizacao_id'):
        op.create_index('ix_item_wms_localizacao_id', 'item_wms', ['localizacao_id'])
    inspector = sa.inspect(bind)
    if _has_table(inspector, 'item_wms') and not _has_index(inspector, 'item_wms', 'ix_item_wms_status'):
        op.create_index('ix_item_wms_status', 'item_wms', ['status'])

    if not _has_table(inspector, 'movimentacao_wms'):
        op.create_table(
            'movimentacao_wms',
            sa.Column('id', sa.Integer(), nullable=False),
    ...
    inspector = sa.inspect(bind)
    if _has_table(inspector, 'movimentacao_wms') and not _has_index(inspector, 'movimentacao_wms', 'ix_movimentacao_wms_item_wms_id'):
        op.create_index('ix_movimentacao_wms_item_wms_id', 'movimentacao_wms', ['item_wms_id'])
    inspector = sa.inspect(bind)
    if _has_table(inspector, 'movimentacao_wms') and not _has_index(inspector, 'movimentacao_wms', 'ix_movimentacao_wms_numero_nota'):
        op.create_index('ix_movimentacao_wms_numero_nota', 'movimentacao_wms', ['numero_nota'])

    if not _has_table(inspector, 'estoque_wms'):
        op.create_table(
            'estoque_wms',
            sa.Column('id', sa.Integer(), nullable=False),
            sa.Column('codigo_item', sa.String(50), nullable=False),
    ...
    inspector = sa.inspect(bind)
    if _has_table(inspector, 'estoque_wms') and not _has_index(inspector, 'estoque_wms', 'ix_estoque_wms_codigo_item'):
        op.create_index('ix_estoque_wms_codigo_item', 'estoque_wms', ['codigo_item'])
    inspector = sa.inspect(bind)
    if _has_table(inspector, 'estoque_wms') and not _has_index(inspector, 'estoque_wms', 'ix_estoque_wms_localizacao_id'):
...
def downgrade():
    ...
    if _has_table(inspector, 'estoque_wms') and _has_index(inspector, 'estoque_wms', 'ix_estoque_wms_localizacao_id'):
        op.drop_index('ix_estoque_wms_localizacao_id', table_name='estoque_wms')
    inspector = sa.inspect(bind)
    if _has_table(inspector, 'estoque_wms') and _has_index(inspector, 'estoque_wms', 'ix_estoque_wms_codigo_item'):
        op.drop_index('ix_estoque_wms_codigo_item', table_name='estoque_wms')
    inspector = sa.inspect(bind)
    if _has_table(inspector, 'estoque_wms'):
        op.drop_table('estoque_wms')

    inspector = sa.inspect(bind)
    if _has_table(inspector, 'movimentacao_wms') and _has_index(inspector, 'movimentacao_wms', 'ix_movimentacao_wms_numero_nota'):
        op.drop_index('ix_movimentacao_wms_numero_nota', table_name='movimentacao_wms')
    inspector = sa.inspect(bind)
    if _has_table(inspector, 'movimentacao_wms') and _has_index(inspector, 'movimentacao_wms', 'ix_movimentacao_wms_item_wms_id'):
        op.drop_index('ix_movimentacao_wms_item_wms_id', table_name='movimentacao_wms')
    inspector = sa.inspect(bind)
    if _has_table(inspector, 'movimentacao_wms'):
        op.drop_table('movimentacao_wms')

    inspector = sa.inspect(bind)
    if _has_table(inspector, 'item_wms') and _has_index(inspector, 'item_wms', 'ix_item_wms_status'):
        op.drop_index('ix_item_wms_status', table_name='item_wms')
    inspector = sa.inspect(bind)
    if _has_table(inspector, 'item_wms') and _has_index(inspector, 'item_wms', 'ix_item_wms_localizacao_id'):
        op.drop_index('ix_item_wms_localizacao_id', table_name='item_wms')
    inspector = sa.inspect(bind)
    if _has_table(inspector, 'item_wms') and _has_index(inspector, 'item_wms', 'ix_item_wms_codigo_item'):
        op.drop_index('ix_item_wms_codigo_item', table_name='item_wms')
    inspector = sa.inspect(bind)
    if _has_table(inspector, 'item_wms') and _has_index(inspector, 'item_wms', 'ix_item_wms_numero_nota'):
        op.drop_index('ix_item_wms_numero_nota', table_name='item_wms')
    inspector = sa.inspect(bind)
    if _has_table(inspector, 'item_wms'):
        op.drop_table('item_wms')

    inspector = sa.inspect(bind)
    if _has_table(inspector, 'localizacao_armazem') and _has_index(inspector, 'localizacao_armazem', 'ix_localizacao_armazem_codigo'):
        op.drop_index('ix_localizacao_armazem_codigo', table_name='localizacao_armazem')
    inspector = sa.inspect(bind)
    if _has_table(inspector, 'localizacao_armazem'):
