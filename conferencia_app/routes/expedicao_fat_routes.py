...
@expedicao_fat_bp.route(
    "/api/expedicao/conf-cega/ordens/<int:cod_ordem_fat>/expedir-sem-conferencia",
    methods=["POST"],
)
@permission_required(PERMISSION)
def expedir_sem_conferencia_fat(cod_ordem_fat):
    """Cria o romaneio de uma ordem que ja saiu faturada SEM conferencia cega.

    Caminho de excecao: como nao houve conferencia, tambem nao existe foto da
    mercadoria nem do cliente — e e justamente a foto do cliente que bloqueia
    a NF de entrar num romaneio pelo caminho normal. Em troca, exigimos a
    justificativa de quem liberou, que acompanha a NF ate o romaneio e ate o
    Registro de expedicao (que nasce Finalizado, sem canhoto a esperar).
    """
    ordem = ExpedicaoOrdemFat.query.filter_by(cod_ordem_fat=cod_ordem_fat).first()
    if not ordem:
        return jsonify({"error": "Ordem de faturamento nao encontrada."}), 404
