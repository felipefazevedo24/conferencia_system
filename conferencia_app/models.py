...
class LogisticaInventarioAjuste(db.Model):
    ...
    SNAPSHOT de quando a divergencia foi detectada - nao recalculados
    depois, pra o gestor sempre ver os mesmos numeros que geraram o alerta,
    mesmo que o saldo/custo do GRV mude enquanto o ajuste ainda esta em
    aberto."""
...
class LogisticaConsumoChapaNesting(db.Model):
    ...
    Workflow simples: Nesting (importado, chapas ja consumidas fisicamente
    pelo corte) -> Concluido (confirmacao manual depois, ex.: baixa de
    estoque conferida)."""
    ...
    status = db.Column(db.String(20), nullable=False, default="Nesting", index=True)  # Nesting | Concluido
