...
from conferencia_app.models import DepositoWMS, EstoqueWMS, ItemNota, ItemWMS, LocalizacaoArmazem, MovimentacaoWMS, WMSReconciliacaoDivergencia, WMSSkuMestre, WMSTarefaOperacional, WMSIntegracaoEvento, WMSInventarioCiclico, WMSPedidoSeparacao, WMSUnidadeLogistica
...
def test_wms_coletor_bipa_sku_lancado_no_postgres_e_endereca_quantidade_parcial(tmp_path):
    ...
    with patch('conferencia_app.services.wms_service.WMSService.consultar_entrada_erp_para_wms', return_value=entrada_pg):
    ...
    with app.app_context():
        ...
        armazenado = ItemWMS.query.filter(ItemWMS.codigo_item == 'SKU-PG', ItemWMS.localizacao_id.isnot(None), ItemWMS.ativo == True).one()
...
def test_wms_detalhe_deposito_e_sku_retorna_visao_completa(tmp_path):
    app = build_test_app(tmp_path)
    client = app.test_client()
    login_admin(client)
    ...
    with app.app_context():
        ...
        db.session.add(loc)
        db.session.flush()
        db.session.add(WMSSkuMestre(codigo_item='SKU-DET', codigo_erp='SKU-DET', unidade='PC', curva_abc='A', endereco_preferencial='AL-DT-01-01', ativo=True))
        item = ItemWMS(
            numero_nota='NF-DET',
            codigo_item='SKU-DET',
            descricao='Item detalhado',
            qtd_recebida=12,
            qtd_atual=12,
            unidade='PC',
        ...
        db.session.add_all([item, pend])
        db.session.flush()
        db.session.add(EstoqueWMS(codigo_item='SKU-DET', localizacao_id=loc.id, qtd_total=12, qtd_separada=2, qtd_bloqueada=1))
        db.session.add(MovimentacaoWMS(item_wms_id=item.id, numero_nota='NF-DET', tipo_movimentacao='Armazenamento', localizacao_destino_id=loc.id, qtd_movimentada=12, usuario='tester'))
        db.session.add(WMSReconciliacaoDivergencia(numero_nota='ERP_SYNC', codigo_item='SKU-DET', qtd_erp=13, qtd_wms=12, diferenca=1, status='Aberta', origem='Teste'))
        db.session.commit()

    dep_resp = client.get('/api/wms/depositos/AL/detalhe')
    assert dep_resp.status_code == 200
    dep_data = dep_resp.get_json()
    assert dep_data['resumo']['skus'] >= 1
    assert any(row['codigo_item'] == 'SKU-DET' for row in dep_data['top_skus'])
    assert any(row['localizacao_codigo'] == 'AL-DT-01-01' for row in dep_data['estoque'])

    sku_resp = client.get('/api/wms/skus/SKU-DET/detalhe')
    assert sku_resp.status_code == 200
    sku_data = sku_resp.get_json()
    assert sku_data['sku']['curva_abc'] == 'A'
    assert sku_data['resumo']['qtd_disponivel'] == 9
    assert sku_data['resumo']['qtd_pendente_enderecamento'] == 4
    assert any(div['status'] == 'Aberta' for div in sku_data['divergencias'])
