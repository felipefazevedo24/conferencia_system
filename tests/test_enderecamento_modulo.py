...
def test_lista_de_enderecos_vem_do_grv_e_falha_nao_derruba_a_tela(app, state, monkeypatch):
    ...
    dados = client.get('/api/enderecamento/saldos').get_json()
    ...
    assert dados['metricas'] == {'materiais': 2, 'enderecos': 2, 'sincronizar': 0}
    assert client.get('/api/enderecamento/saldos?busca=B').get_json()['total'] == 2  # endereço
